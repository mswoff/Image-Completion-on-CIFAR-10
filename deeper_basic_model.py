...
def init_params_deep_basic():
    ...
    W3 = tf.get_variable("W3", [3, 3, filters2, filters3], initializer=tf.contrib.layers.xavier_initializer(seed=0))
    W4 = tf.get_variable("W4", [3, 3, filters3, filters4], initializer=tf.contrib.layers.xavier_initializer(seed=0))
    W5 = tf.get_variable("W5", [1, 1, filters4, 3], initializer=tf.contrib.layers.xavier_initializer(seed=0))

    return {"W1": W1, "W2": W2, "W3": W3, "W4": W4, "W5": W5}

def forward_prop_deep_basic(X, parameters):
    W1 = parameters['W1']
    W2 = parameters['W2']
    W3 = parameters['W3']
    W4 = parameters['W4']
    ...
    # A5 = tf.nn.sigmoid(Z5)
    clip = tf.clip_by_value(A5, 0.0, 1.0)
